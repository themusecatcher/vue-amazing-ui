...
| 参数 | 说明 | 类型 | 默认值 |
| :-- | :-- | :-- | :-- |
| items | 滚动文字数组，`single` 为 `true` 时，类型为 `TextScrollItem`；多条文字水平滚动时，数组长度必须大于等于 `amount` 才能滚动 | [TextScrollItem](#item-type)[] &#124; [TextScrollItem](#item-type) | [] |
...
| 名称 | 说明            | 类型                               |
| :---- | :--------------- | :--------------------------------- |
| click | 点击标题时的回调 | (item: [TextScrollItem](#item-type)) => void |
