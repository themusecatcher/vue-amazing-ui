...
<template>
  ...
  <div v-else class="m-progress-circle" :style="`width: ${totalWidth}; height: ${totalWidth};`">
    <svg class="u-progress-circle" viewBox="0 0 100 100">
      <path :d="path" stroke-linecap="round" :stroke-width="strokeWidth" :style="`stroke-dasharray: ${perimeter}px, ${perimeter}px;`" fill-opacity="0" class="u-progress-circle-trail"></path>
      <path :d="path" stroke-linecap="round" :stroke-width="strokeWidth" :style="`stroke-dasharray: ${(percent / 100) * perimeter}px, ${perimeter}px;`" :opacity="percent === 0 ? 0:1" fill-opacity="0" class="u-progress-circle-path" :class="{success: percent >= 100}"></path>
    </svg>
    <template v-if="showInfo">
