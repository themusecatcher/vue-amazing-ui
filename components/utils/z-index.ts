 ...
 * 浮层层级（z-index）管理层
 *
 * 解决的问题：甲类（锚点跟随型）与乙类（视口固定 / 全屏型）浮层此前各自硬编码层级，跨线嵌套必然出错
 * —— 例如 `Modal`（1000）里放 `Tooltip`（999）时浮层会被遮罩盖住。
 *
 * 机制：`ConfigProvider` 传入 `baseZIndex` 后建立本管理器并向下注入，甲、乙两条线共同消费：
 * - 每个层在「出现」时向管理器领取槽位（`allocate`），分配点恒在当前所有已占层之上，即**后出现者在上**；
 * - 层消失 / 组件卸载时归还槽位（`release`），最上层归还后下一次分配即回落，数值随「同时可见的层数」增长；
 * - 未传 `baseZIndex` 时管理器不存在，各组件回退到各自的默认层级（见下方 `FLOATING_LAYER_Z_INDEX`）。
 */
...
export const FLOATING_LAYER_Z_INDEX = {
  ...
  image: 1070,
  /** 下拉面板族：`Select` / `AutoComplete` / `Dropdown` / `Cascader` */
  select: 1050,
...
export interface ZIndexManager {
   ...
   * 领取一个槽位
   *
   * @param span - 需要连续占用的段数（如 Modal 的遮罩与弹窗共 2 段）；可选，默认 1
   * @returns 本层的起始层级
   */
 ...
 * 供子树内的浮层在承载层关闭时收起自身。
 *
 * 为什么需要：容器关闭**不卸载内容**（默认 `destroyOnClose: false`），其内的 `Select` 下拉也不会随之
 * 卸载 —— 下拉关闭只依赖触发器（input）blur，而容器关闭只是把内容 `display: none`、**不派发 blur**，
 * 于是下拉**停留在打开态**：容器已关、下拉仍悬浮在页面上，且仍占着层级槽位。容器再次打开时会按
 * 「后出现者在上」重新领取层级，从而越过这个仍占位的内部浮层 → 形成「子层在父层之下」的反序层级
 * （下拉被遮罩 / 弹窗盖住），同时让分配点被无谓抬高。子树内的浮层据此在承载层关闭时收起自身
 * （收起即归还槽位），恢复「数值随同时可见层数增长」。
 */
 ...
 * 组合式函数：获取本层的层级值
 *
 * - 未注入管理器（`ConfigProvider` 未传 `baseZIndex`）时恒为 `fallback`，即该组件的**默认层级**
 *   （见 `FLOATING_LAYER_Z_INDEX` 与文档《浮层层级管理 · 各组件默认层级》表）；
 * - 已注入管理器时挂载即领取槽位，组件每次「出现」可再调用 `allocate()` —— 内部先归还旧槽位再领取，
 *   使同一组件重新打开时位于其它已打开层之上；卸载时自动归还，避免数值无限增长；
 * - 位于承载层（Modal / Drawer / Dialog）内部时，还需在承载层关闭时收起自身（见
 *   `Z_INDEX_CONTAINER_OPEN_KEY`），否则会以「已占层」的身份被重新打开的承载层反超。
 *
 * @param fallback - 无管理器时的回退层级（= 该组件的默认层级）
 * @param span - 本层需连续占用的段数（内部还有相对分层的组件传更大值），默认 1
 * @param options - 选项；`allocateOnMount: false` 时挂载不领取，由消费者按「出现」驱动
 * @returns 层级 ref 与领取 / 归还方法
 */
