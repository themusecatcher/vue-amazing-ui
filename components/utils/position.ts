 ...
 *
 * 本文件承载两套契约：
 * - **浮层定位内核**（`useFloating`）：求解层 + 同步层，负责「算 + 输出 + 同步」。求解层以查表方式
 *   完成 flip / 次轴 align 自适应 / offset 位移 / transformOrigin；同步层负责全链滚动监听 +
 *   帧合并 + 视口矩形测量。内置浮层组件（`Tooltip` / `Select` / `AutoComplete` / `Slider`）均已迁到它；
 * - **`getFloatingBoundaryRect`**：遮挡边界测量（`boundary: 'scrollParent'` 口径），由内核内部复用。
 *
 * 两者均服务于组件库内部浮层实现，不对外导出：其行为与组件内部 DOM 契约（两层定位容器、
 * `data-va-floating-mount` 挂载点、`z-index` 注入协议）强绑定，公开即把内部实现固化为公开 API，
 * 故仅从 barrel 供组件内部引用。
 */
 ...
 * 弹出层 Teleport 到 body / 具名容器时不受中间滚动容器 overflow 裁剪，flip 边界应为视口，
 * 避免空间充足却意外翻转。
 *
 * 由 `useFloating` 的 `boundary: 'scrollParent'` 口径复用（求解边界与锚点可见性判定共用同一口径）；
 * 视口尺寸由内核在每一帧内统一测量后传入，避免同帧内重复测量。
 *
 * @param scrollTarget 最近的可滚动父元素（内核由 `getScrollParent(anchor)` 取得），为 null 时以视口为界
 * @param panel 浮层面板元素，用于判断是否真的被 scrollTarget 裁剪
 * @param viewportWidth 视口宽度（由内核逐帧测量）
 * @param viewportHeight 视口高度（由内核逐帧测量）
 * @returns {FloatingBoundaryRect} 遮挡边界矩形相对视口的四条边
 */
 ...
 *
 * 由三部分构成：
 * 1. 求解层：查表式 flip / 次轴 align 自适应 / offset 位移 / transformOrigin 查表（不含 overlap
 *    分支，本库无 overlap 需求）；
 * 2. 测量层：用 `position: fixed; top/right/bottom/left: 0` 的隐藏元素测真实视口矩形；支持三种
 *    锚点（DOM 元素 / 虚拟锚点 / x-y 手动定位）；浮层尺寸取布局尺寸（`offsetWidth / offsetHeight`，
 *    避开缩放动画期间的 rect 失真）；
 * 3. 同步层：全链滚动祖先监听 + 帧合并 + 字体就绪，并对外提供 `sync()` 供锚点持续移动场景主动
 *    强制重对齐。相对常见实现，本库把 flip 与 shift 拆成两个独立开关（`!flip` 时不连带跳过
 *    align 自适应与 shift）。
 *
 * 另外合并了本库的业务侧能力：遮挡边界口径（`boundary`，复用 `getFloatingBoundaryRect`）、等宽 / 最小等宽
 * （`matchTriggerWidth`）。「3 态水平对齐」（贴左 / 贴右 / 贴视口左边缘）无需单独参数：前两态即
 * placement 的次轴后缀，第三态由 `shift` 的 align 自适应 + 位移（`deriveOffset`）实现，与
 * `boundary` 口径共同决定贴哪一侧。
 * ============================================================================================== */
 ...
 *
 * 内核内部统一映射为 kebab 命名（`top` / `top-start` / `left-start` …），对外入参、出参一律使用
 * 本命名，保证组件公开 props 不受内核命名影响。
 */
...
/** kebab 命名：主轴（次轴缺省即 center）或主轴 + 对齐后缀 */
 ...
 * 内部坐标系下的矩形
 *
 * ⚠️ **这不是纯坐标，勿按直觉改写**：改写必须逐 placement × 逐边界条件自证等价，否则会引入比
 * 现状更隐蔽的缺陷。
 ...
 * 恒表示「该方向上的可用空间」，翻转判定与次轴 align 自适应全部建立在它之上。
 */
 ...
 * 元素首次使用时惰性创建并复用（模块级缓存）。
 *
 * @returns {DOMRect | null} 视口矩形；SSR（Node）或无 body 时返回 null
 */
...
/** 换算所需的矩形字段集（DOMRect 或同形对象，兼容虚拟锚点返回的普通对象） */
type FloatingRectLike = Pick<DOMRect, 'left' | 'top' | 'right' | 'bottom' | 'width' | 'height'>

/**
 * 虚拟锚点
 *
 * 只需提供 `getBoundingClientRect`，不要求是真实 DOM 元素：`Tour` / `Anchor` / Table 列筛选等
 * 需要「指向任意位置」的场景，可直接喂入一个合成矩形。
 */
...
/**
 * 把视口矩形换算为内部坐标系矩形（见 {@link FloatingRect} 的口径说明）
 *
 * @param {FloatingRectLike} rect 视口矩形
 * @returns {FloatingRect} 内部坐标系下的矩形
 */
 ...
 *
 * 浮层尺寸取 `offsetWidth / offsetHeight` 而非 `getBoundingClientRect`，因为进入 / 离开的缩放动画
 * 会改变 `transform`，使 rect 返回缩放后的视觉尺寸（动画期间偏小 → 翻转判定与次轴位移算错）。
 * 求解只用浮层的宽高，故此处直接以布局尺寸覆盖。
 *
 * @param {HTMLElement} el 浮层元素
 * @returns {FloatingRect} 内部坐标系下的浮层矩形
 */
 ...
 * 皮肤层缩放动画的 `transform` 影响（`getBoundingClientRect` 会受影响，见
 * {@link getFollowerFloatingRect}），因此浮层在动画期间也能量到正确尺寸。
 *
 * @param {HTMLElement} el 浮层元素
 * @param {{ width: number, height: number }} fallback 量不到时的回退值（offset 尺寸）
 * @returns {{ width: number, height: number }} border box 尺寸
 */
 ...
 *
 * 求解层对 `targetRect` 的 left / top / right / bottom 一律按**可用空间**解读（见
 * {@link FloatingRect}），因此换边界只需换空间的参照物：四条边都改为「到边界对应边的距离」。
 * 边界取视口时该换算等价于恒等（视口原点为 0，宽度即右边界），故可统一走一条代码路径。
 *
 * 注意：`getOffset` 需要的是「相对定位容器的坐标」，因此那里仍传视口口径的矩形 —— 两者不可混用。
 *
 * @param {FloatingRect} rect 视口口径的矩形
 * @param {FloatingBoundaryRect} boundary 遮挡边界
 * @returns {FloatingRect} 以遮挡边界为口径的矩形
 */
 ...
 * 目标（锚点）是否已完全离开「自身可见区」
 *
 * 判定用「矩形与可见区无交集」而非「尺寸为 0」：锚点只要还有 1px 落在可见区内，就仍需正常翻转与内推
 * （例如触发器贴视口底部、两侧都放不下时把浮层推回边界内）。
 *
 * @param {FloatingRect} targetRect 锚点矩形（内部坐标系，left / top 即视口坐标）
 * @param {FloatingBoundaryRect} visibleRect 锚点自身的可见区（最近可滚动祖先 ∩ 视口），视口坐标的四条边
 * @returns {boolean} 锚点完全离开可见区时为 true
 */
 ...
 * 的剩余空间」，与 {@link FloatingRect} 的约定自洽（视口原点非零的场景由 toFloatingRect 统一换算）。
 *
 * @param {number} x 视口坐标 x（通常为鼠标事件 clientX）
 * @param {number} y 视口坐标 y（通常为鼠标事件 clientY）
 * @returns {FloatingRect} 点锚点的内部坐标系矩形
 */
 ...
 *
 * 与本库其他实现约定的差异（均为显式决策）：
 * 1. `!flip` 时不连带跳过 align 自适应与 shift —— 本库把 flip 与 shift 拆成两个独立开关：只有两者
 *    同时关闭才跳过，避免「关闭主轴翻转」时连次轴兜底一起失效；
 * 2. 「放不下」判定额外计入 `extraMainAxisSize`（主轴间距等浮层盒之外仍被占用的空间）——
 *    只比较浮层自身尺寸会漏算间距，浮层会带着间距溢出边界。取 0 时退化为「只比较浮层自身尺寸」；
 * 3. **主轴翻转口径**：**当前侧放不下，且对侧空间更大**才翻转 ——
 *    即绝不跨轴，也绝不在空间更小的一侧展开。两侧都放不下时浮层必然溢出遮挡边界，故由调用方按
 *    {@link getMainAxisOverflowOffset} 把浮层推回边界内，保证浮层至少完整可见。
 *
 * @param {KebabPlacement} placement 期望方向（kebab 命名）
 * @param {FloatingRect} targetRect 锚点矩形（内部坐标系，可按遮挡边界为口径）
 * @param {FloatingRect} followerRect 浮层矩形（内部坐标系；求解只用其 width / height）
 * @param {boolean} shift 是否做次轴对齐微调（偏移量）
 * @param {boolean} flip 是否允许主轴同轴翻转
 * @param {number} [extraMainAxisSize = 0] 主轴方向浮层盒之外仍占用的尺寸（px），仅参与「放不下」判定
 * @returns {{ placement: KebabPlacement, top: number, left: number }} 实际方向与主 / 次轴微调量（px）
 */
export function getPlacementAndOffsetOfFollower(
  ...
  // 主轴翻转：仅同轴（position ↔ oppositionPositions[position]）—— 「当前侧放不下，且对侧空间更大」
  // 才翻转（绝不跨轴，也绝不在空间更小的一侧展开）。两侧都放不下时浮层必然溢出遮挡边界，由调用方
  // 按 getMainAxisOverflowOffset 推回边界内，因此这里不必为了「避免裁切」而放宽翻转条件（那会让
  // 方向随两侧空间的微小变化来回跳）
 ...
 *
 * @param {KebabPlacement} placement 实际方向（kebab 命名）
 ...
 *
 * 逐 case 给出：位移全部写成「锚点边缘 − 容器原点 [+ 对齐微调]」，自身尺寸则由**百分比位移**承担，
 * 因此**浮层尺寸变化时无需重算**。
 *
 * **位移用 CSS 独立属性 `translate` 而非 `transform`**：皮肤层的缩放动画用的是独立属性 `scale`，
 * 而独立变换属性的复合顺序为 `translate → rotate → scale → transform`。若定位写在 `transform` 里，
 * `scale` 会与之相乘，使位移量被一并缩放（动画期间浮层整体偏移 `(1 - scale) × 位移量`，观感上像从
 * 错误的一角弹出）；写在 `translate` 里则 `scale` 只作用于已就位的盒子，动画期间位置保持不动。
 *
 * @param {KebabPlacement} placement 实际方向（kebab 命名）
 * @param {{ left: number, top: number }} offsetRect 定位容器的视口矩形（只用其 left / top 作坐标原点）
 * @param {FloatingRect} targetRect 锚点矩形（内部坐标系）
 * @param {number} offsetTopToStandardPlacement 主轴方向的对齐微调量（top）
 * @param {number} offsetLeftToStandardPlacement 主轴方向的对齐微调量（left）
 * @returns {{ top: string, left: string, translate: string }} 浮层的内联定位样式值
 */
 ...
 *
 * **为什么需要**：位移写成百分比是为了「浮层尺寸变化时无需重算」（见 {@link getOffset}），
 * 但浮层尺寸常带小数（卡片宽度由文本撑开，如 `118.15625px`），百分比位移随之落在小数像素上。
 * 此时浏览器把浮层的**背景盒**对齐到整数像素绘制，而**自带 transform 节点的后代**（如箭头的
 * 旋转盒）仍按其精确布局位置绘制 —— 两者边缘在相接处错开不足 1px，留下一条淡色细缝
 * （`Tooltip` / `Slider` 的 `left` / `right` 方向肉眼可见；`top` / `bottom` 方向因浮层高度
 * 通常为整数而不出现）。
 *
 * **做法**：只补掉「不足 1px 的小数部分」，百分比本身仍承担「随浮层尺寸自适应」，尺寸变化时
 * 依旧无需重算；补足后浮层落在整数像素上，背景边缘与箭头边缘重新对齐（实测细缝消失）。
 *
 * @param {string} translate 百分比位移（形如 `'-100% -50%'` / `'0 -100%'` / `''`）
 * @param {{ width: number, height: number }} size 浮层 border box 尺寸（百分比位移的参照盒）
 * @returns {string} 取整后的位移；无需修正时原样返回
 */
 ...
 *
 * @param {string} token 单轴位移（`0` / `-50%` / `-100%`）
 * @param {number} refSize 该轴的参照尺寸（px）
 * @returns {string} 取整后的位移；非百分比 / 参照尺寸不可用 / 已在整数像素上时原样返回
 */
 ...
 *
 * 主轴间距由浮层自身的视觉盒之外的空间承担，此处把它显式化为 `offset` 参数，
 * 叠加进 getOffset 的对齐微调量中（方向符号随实际方向确定）。
 *
 * @param {KebabPlacement} placement 实际方向（kebab 命名）
 * @param {number} offset 主轴间距（px）
 * @returns {{ top: number, left: number }} 叠加到 getOffset 微调量上的偏移
 */
 ...
 *
 * @param {KebabPlacement} placement 实际方向（kebab 命名）
 * @param {FloatingRect} targetRect 锚点矩形（视口口径；left / top 为真实坐标）
 * @param {FloatingRect} followerRect 浮层矩形（只取主轴尺寸）
 * @param {FloatingBoundaryRect} boundary 遮挡边界（视口口径）
 * @param {number} gap 主轴间距（px）
 * @returns {{ top: number, left: number }} 叠加到 getOffset 微调量上的主轴位移（全 0 表示无需调整）
 */
 ...
 *
 * @param {HTMLElement} panel 浮层元素
 * @param {number} triggerWidth 锚点宽度（px）
 * @param {'width' | 'minWidth' | number | false} match 尺寸关系
 */
 ...
 * 收集锚点的**全链**滚动祖先
 *
 * 用 while 逐级向上遍历，把每一层可滚动祖先都纳入监听 —— 只取最近的一个时，锚点处于嵌套双层
 * 滚动容器内时外层滚动不触发跟随。视口滚动的事件目标为 window
 * （元素级 scroll 不冒泡，documentElement 收不到），故 documentElement 统一替换为 window。
 *
 * @param {HTMLElement} el 起点元素（锚点）
 * @returns {(HTMLElement | Window)[]} 需要监听 scroll 的全链目标
 */
 ...
 * （宿主的样式更新发生在本轮 post 回调之后），此时测得的尺寸全为 0 —— 详见 {@link applyPosition}。
...
export interface UseFloatingOptions {
   ...
   * 浮层的定位参照容器
   *
   * 宿主自持两层结构（`absolute` 容器 + `absolute` 浮层）时可传入容器引用；本库的宿主 DOM 由
   * `<Popup>` 渲染，故容器由宿主传入。浮层的 `top` / `left` 即相对该容器的 padding box 计算
   * （容器需满足：`position: absolute`、无 padding / border，作为定位原点）。容器矩形按**实时测量**，
   * 因此 Teleport 到 body 与 `to: false` 就地渲染两种情况共用同一套求解，无需内核区分。
   */
...
export interface UseFloatingReturn {
  /** 浮层内联定位样式：top / left / transform（尺寸交由百分比 transform 承担） */
 ...
 * - 同步：锚点所在**全链**滚动祖先 + 视口 resize + 字体就绪自动重算，另有 `sync()` 供
 *   锚点持续移动（拖动）场景主动强制重对齐。
 *
 * 锚点三选一，按 `point` → `virtualAnchor` → `anchor` 的优先级取第一个可用者。
 *
 * @param {Ref<HTMLElement | null>} panelRef 浮层元素（定位样式的落点）
 * @param {UseFloatingOptions} options 锚点（三选一）、参照容器与求解参数
 * @returns {UseFloatingReturn} 定位样式、实际方向、动画原点、主动重算与帧级重对齐方法
 */
export function useFloating(panelRef: Ref<HTMLElement | null>, options: UseFloatingOptions): UseFloatingReturn {
  ...
  // 计算并落盘定位样式：单次求解 = 量 → 求方向与微调量 → 求 top / left / transform
  function applyPosition(): void {
    ...
    // 浮层尚未渲染：典型场景是宿主用 `v-show` 控制的浮层「再次显示」——`enabled` 翻真的首个求解
    // tick 里浮层仍是 `display: none`（宿主的样式更新发生在本轮 post 回调之后），offsetWidth /
    // offsetHeight 双双为 0。此时若照常求解，「下方永远放得下」会让主轴既不翻转也不内推，且此后
    // 不会再有任何 trigger 触发重算（现象：首次打开正常翻转到上方，二次打开却向下展开并被裁切）。
    // 故本轮不落盘样式，改到后续帧重试（带上限，避免浮层长期未渲染时空转）
    ...
    // 锚点可见性守卫：锚点已完全离开可见区时不做主轴翻转与内推 —— 浮层按期望方向原样跟随锚点移出边界
    // （缺该守卫时浮层会钉在视口边缘不随锚点离场，观感像 `position: fixed`）。
    // 可见区取「锚点最近可滚动祖先 ∩ 视口」：锚点被滚动容器裁掉时同样视为离屏。
    ...
    // 两侧都放不下时（翻转后仍放不下）浮层必然溢出遮挡边界而被裁切，按主轴方向推回边界内；
    ...
    // 百分比位移取整：浮层尺寸常带小数，否则背景边缘会与箭头等自带 transform 的后代边缘错开 <1px，
    // 在相接处留下淡色细缝（见 snapTranslate）。translate 为空（无位移）时无需测量浮层尺寸
  ...
  // 锚点来源 / 浮层 / 启用态变化时重建监听并立即定位：post flush 保证 DOM 已就绪且在浏览器绘制前完成，
  // 避免首帧错位闪烁
  watch(
