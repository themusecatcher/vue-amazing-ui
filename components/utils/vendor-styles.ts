 ...
 * vendorStylesDir 固定目录，再由构建期生成的「每组件样式入口」按需引用。
 *
 ...
 * 1. vite.config.ts 的 stylePostBuildPlugin（copyVendorStyles）—— 按 source 从 node_modules 复制到产物 vendorStylesDir 目录
 * 2. build/generate-style-entries.ts —— 按 vendorStylesByComponent 把 target 写进对应组件的样式入口
 */
 ...
 * 供 build/generate-style-entries.ts 按组件名查表写入样式入口；由 vendorStyles 派生，保证两处始终一致
 */
